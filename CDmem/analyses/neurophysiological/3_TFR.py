...
ANALYSIS PIPELINE:
    ...
    Part 2 (group level, per contrast):
        5. Load per-subject TFR
        6. Split trials by memory outcome (± control condition)
        7. Trial-balance conditions (100 random subsamples)
        8. Average across trials per condition
        9. Baseline correct in dB (-0.5 to -0.2 s)
        10. Cluster-based permutation test (channel × frequency × time)
        11. Publication-ready figures (TFR maps + topoplots)
...
ch_names_all = group_tfr_data[first_sub]['ch_names']
n_channels = len(ch_names_all)
n_freqs = len(FREQS)
n_times_full = len(tfr_times)

print(f"  Channels: {n_channels}, Frequencies: {n_freqs}, "
...
for contrast in contrasts:
    ...
    # Stack into arrays: (n_subjects, n_channels, n_freqs, n_times)
    ...
    #   where n_vertices = n_channels × n_freqs (the spatial dimension).
    ...
    # Shape: (n_subjects, n_channels, n_freqs, n_times)

    # Crop to statistical time window
    # FieldTrip: cfg.latency = [0, 4];
    X_diff_stat = X_diff[:, :, :, stat_time_idx]
    # Shape: (n_subjects, n_channels, n_freqs, n_times_stat)

    # Crop to statistical frequency range
    # FieldTrip: cfg.frequency = [2, 40];
    X_diff_stat = X_diff_stat[:, :, stat_freq_idx, :]
    # Shape: (n_subjects, n_channels, n_freqs_stat, n_times_stat)
    ...
    X_for_test = X_diff_stat.transpose(0, 3, 1, 2).reshape(
        n_valid, n_times_stat, n_channels * n_freqs_stat
    )
    ...
    # T_obs shape: (n_times_stat, n_channels × n_freqs_stat)
    # Reshape back to (n_times_stat, n_channels, n_freqs_stat)
    T_obs_3d = T_obs.reshape(n_times_stat, n_channels, n_freqs_stat)
    # Transpose to (n_channels, n_freqs_stat, n_times_stat)
    ...
    for clust_mask in clusters:
        ...
        mask_3d = clust_mask.reshape(n_times_stat, n_channels, n_freqs_stat)
    ...
    for i, (mask, pval) in enumerate(zip(cluster_masks_3d, cluster_pv)):
        ...
        ch_list = [ch_names_all[j] for j in np.where(ch_in)[0]]
    ...
    np.savez_compressed(
        ...
        ch_names=np.array(ch_names_all),
    ...
    combined_mask = np.zeros((n_channels, n_freqs_stat, n_times_stat),
                             dtype=bool)
